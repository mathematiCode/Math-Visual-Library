mathVisual.fractionDivisionBar = (svg, dividend, divisor, attributes) => {
  ...
  // If both numerator and denominator are 0 (fraction part left blank), set denominator to 1 to avoid division by zero
  if (dividend.numerator == 0 && dividend.denominator == 0) {
    dividend.denominator = 1;
  }
  if (divisor.numerator == 0 && divisor.denominator == 0) {
    divisor.denominator = 1;
  }

  // If numerator is provided but denominator is empty (0), ignore the fraction by setting numerator to 0
  if (dividend.numerator != 0 && dividend.denominator == 0) {
  ...
  // If numerator equals denominator (like 1/1, 2/2), convert to whole number
  if (dividend.numerator != 0 && dividend.numerator == dividend.denominator) {
    dividend.wholeNum += 1;
    dividend.numerator = 0;
  }
  if (divisor.numerator != 0 && divisor.numerator == divisor.denominator) {
    divisor.wholeNum += 1;
    divisor.numerator = 0;
  }
  ...
  } else if (dividend.denominator == 0 && dividend.numerator != 0) {
    return alert(`All denominators must be positive.`);
  } else if (divisor.denominator == 0 && divisor.numerator != 0) {
    return alert(`All denominators must be positive.`);
  ...
  if (dividend.wholeNum == 0) {
    ...
    if (dividend.numerator != 0 && dividend.denominator != 0) {
      fractionalPart = (interval / dividend.denominator) * dividend.numerator;
    }
    dividend.size = parseFloat(
      (interval * dividend.wholeNum + fractionalPart).toFixed(2) -
